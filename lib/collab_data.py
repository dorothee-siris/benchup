...
Pure functions, no Streamlit import. `breadth_jaccard` reads `subs["l3"]
["share"]` ('s topic-grain substrate, tree-invariant since
topics are the base grain the tree never re-buckets) -- the SAME matrix
`lib/engine/lenses.py`'s L3 lens scores overlap on.

/ (2026-09-03): the "topics-together" table
...
def _topic_membership(ctx: dict, subs: dict, idx: int, min_full: int) -> "np.ndarray":
    ...
    is not breadth; the relationship block passes 2 and says so)."""
    if min_full <= 0:
...
# parquet`/`collab_pair_topics.parquet` pair artefacts. Both
...
PAIR_TOPICS_TOP_N = 100  # -12: top-100 joint topics per pair by vol_total (slider-ready, up to this cap)
...
def breadth_jaccard(ctx: dict, subs: dict, a: str, b: str, min_full: int = 0) -> dict:
    ...
    deliberately answers a DIFFERENT question from the deleted "topics-
    together" table (topic
